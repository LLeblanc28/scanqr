...
app.post("/add-barcode", async (req, res) => {
  ...
  try {
    const [rows] = await connection
      .promise()
      .query("SELECT QTE FROM cbarre WHERE IdBarre = ?", [IdBarre]);

    if (rows.length > 0) {
      const newQte = Number(rows[0].QTE) + qteNumber;
      await connection
        .promise()
        .query("UPDATE cbarre SET QTE = ?, date = ? WHERE IdBarre = ?", [
          newQte,
          date,
          IdBarre,
        ]);
      ...
      await connection
        .promise()
        .query("INSERT INTO cbarre (IdBarre, QTE, date) VALUES (?, ?, ?)", [
          IdBarre,
          qteNumber,
          date,
        ]);
...
app.get("/get-count", (req, res) => {
  ...
  connection.query(countSql, (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération du nombre d'entrées" });
    }
